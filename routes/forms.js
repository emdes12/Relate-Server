...
router.post("/", authorization, async (req, res) => {
  ...
  try {
    ...
    const { title, description, closingDate, submitText, color, fields, completionMessage } =
      req.body;
    ...
    const formResult = await db.query(
      ...
      [user_id, title, description, color, closingDate, submitText, completionMessage]
    );
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    const form = await db.query(`SELECT * FROM forms WHERE form_id = $1`, [formId]);
...
router.get('/', authorization, async (req, res)=>{
    ...
    if(userInfo.user_permission !== 'admin') {
        const staffEmployee = await db.query("SELECT user_id from staffs where staff_email=$1", [userInfo.user_email]);
        admin_id = staffEmployee.rows[0].user_id
    } else {
        admin_id = req.user
    }

    const forms = await db.query("SELECT * FROM forms WHERE user_id=$1  ORDER BY created_at DESC", [admin_id])
